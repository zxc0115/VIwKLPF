...
from ptb import PTB
from KLPFwas.KLPFwas_model import KLPFwas
...
latent_dim = 32
dropout_rate = 0.5
model = KLPFwas(vocab_size=datasets['train'].vocab_size,
...
sio.savemat("KLPFwas.mat", tracker)


# In[42]:


rand_sample = torch.randn(16,32).cuda()
print(rand_sample.shape)
sym = model.inference(rand_sample).cpu().numpy()
print(sym)
sentence = ''
for i in range(sym.shape[0]):
    for j in range(sym.shape[1]):
        if sym[i][j] == 3: break
        sentence += id_to_word[sym[i][j]] + ' '
    print(sentence)
    print('---------------------------------')
    sentence = ''
