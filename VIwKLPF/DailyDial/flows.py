...
class myIAF(BaseFlow):
    
    def __init__(self, dim, hid_dim, context_dim, num_layers,
                 activation=nn.ELU(), realify=nn_.sigmoid, fixed_order=False):
        super(myIAF, self).__init__()
        self.realify = realify
    ...
    def forward(self, inputs):
        #print(len(inputs))
        x, logdet, context, mu = inputs
        out, _ = self.mdl((mu, context))
        if isinstance(self.mdl, iaf_modules.cMADE):
            mean = out[:,:,0]
            lstd = out[:,:,1]
        ...
        #elif self.realify == nn_.sigmoid:
        gamma = 0.1
        alpha = abs((-std+1.0) * mean)
        weight = alpha + gamma # 32*200
        x_ =  alpha * x1_i + x + x1_i * gamma # 32*200
        weight = weight.unsqueeze(dim=2).cuda()
        I = torch.eye(x_.shape[1]).cuda()
        a = torch.transpose(np.repeat(a.unsqueeze(dim=0),x_.shape[0],axis=0),1,2).cuda()
        logdet = logdet.cuda()
        #elif self.realify == nn_.sigmoid2:
        #    x_ = (-std+2.0) * mean + std * x
        logdet_ =  logdet @ ((weight * a) + I)
        return x_, logdet_, context, mu
